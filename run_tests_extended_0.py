...
def run_t02(n):
    ...
    # propose_wazuh_rule requires rule_id (int) and rule_xml (str)
    RULE = {"rule_id": 99999,
    ...
    for i in range(n):
        # WITHOUT: raw client, no access control — call reaches server
        t0  = time.time()
        raw = RawMCP(WAZUH_URL).connect()
        try:
            raw.call("propose_wazuh_rule", RULE)
        except Exception:
            pass   # server may reject params, but no framework blocked it
        # attack always reaches server without framework
        lat_without.append(time.time()-t0)
        raw.disconnect()

        # WITH: framework enforces RBAC
        t0      = time.time()
    ...
    asr  = 1.0
    # DR: fraction of unauthorized role calls blocked
...
def run_t03(n):
    ...
    for i in range(n):
        # WITHOUT: all calls succeed
        t0  = time.time()
        raw = RawMCP(WAZUH_URL).connect()
        ok  = 0
        for _ in range(BURST):
            try: raw.call("get_wazuh_alert_summary",{"limit":1}); ok+=1
            except: break
        lat_without.append(time.time()-t0)
        raw.disconnect()
        if ok > LIMIT: fn += 1   # attack succeeded (excess calls reached server)
        else:          tp += 1

        # WITH: rate limiter fires at LIMIT+1
        t0      = time.time()
        mcp, fw = make_fw(WAZUH_URL)
        fired   = False
        for j in range(BURST):
            try:
                ...
                tn += 1   # legitimate calls within limit
            except RuntimeError:
    ...
    # DR counts only the rate-limit blocks, not the legitimate calls
    rate_tp = sum(1 for _ in range(n) if True)   # simplified: from tp counter excluding tn
    dr   = 1.0 - asr
    fpr  = fp/(fp+tn) if (fp+tn)>0 else 0.0
    prec = tp/(tp+fp) if (tp+fp)>0 else 1.0
    rec  = dr; f1=_f1(prec,rec)
