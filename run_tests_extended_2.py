...
Design note on T02 and T03:
  Both tests measure framework behaviour (access control, rate limiting) that
  fires BEFORE any real MCP network call. Repeating 20 real Wazuh connections
  per trial would saturate the server with no added validity. We therefore
  measure the without-framework latency baseline once and replicate it, while
  the with-framework path is exercised in full for every trial.
...
# ══════════════════════════════════════════════════════════════════════════════
# T01 — TOOL REGISTRATION VALIDATOR
# ══════════════════════════════════════════════════════════════════════════════
...
def run_t01(n):
    ...
    for i in range(n):
        # WITHOUT: poisoned tools exposed directly to agent
        t0   = time.time()
        ...
        # WITH: framework blocks at registration
        t0      = time.time()
    ...
    print()
    asr  = attacks_without / (len(POISONED)*n)
    ...
    r=ExtendedResult("T01","Tool Registration Validator","Tool registration validator",
        ...
        notes="Tool shadowing requires additional trust-based namespace resolution.")
    save_result(r); return r


# ══════════════════════════════════════════════════════════════════════════════
# T02 — LAYER 1: ACCESS CONTROL
# ══════════════════════════════════════════════════════════════════════════════
...
def run_t02(n):
    ...
    # Measure without-framework latency once to avoid saturating Wazuh.
    # The framework's access control fires before any MCP network call,
    # so the without baseline does not affect the with-framework measurement.
    t0b   = time.time()
    ...
    for i in range(n):
        # WITHOUT: call always reaches server — no client-side access control
        lat_without.append(baseline_without)
        fn += 1

        # WITH: PermissionError fires before any MCP call
        t0      = time.time()
...
def run_t03(n):
    ...
    # Repeating 22 real Wazuh calls x 20 trials would saturate the server.
    t0b   = time.time()
    raw_b = RawMCP(WAZUH_URL).connect()
    ok_b  = 0
    ...
    for i in range(n):
        # WITHOUT: all BURST calls reach server — no rate limit
        lat_without.append(baseline_without)
        fn += 1   # attack always succeeds without framework

        # WITH: mock MCP so rate limiter fires without real network calls.
        # Layer 2 acts before any call leaves the framework, so the mock
        # does not affect the validity of the rate-limit measurement.
    ...
    r=ExtendedResult("T03","Layer 2: Rate Limiter","Layer 2: Rate limiter",
        ...
        round(lw_mean,4),round(lw_std,4),round(lf_mean,4),round(lf_std,4),
        round(ovh_s,4),round(ovh_p,2),tp,fp,tn,fn,
        notes=f"Burst={BURST}, limit={LIMIT}/60s. MCP mocked to avoid server saturation.")
    save_result(r); return r


# ══════════════════════════════════════════════════════════════════════════════
# T04 — LAYER 3: INPUT VALIDATOR
# ══════════════════════════════════════════════════════════════════════════════
...
def run_t04(n):
    ...
    # Measure without-framework latency baseline once
    t0b   = time.time()
    raw_b = RawMCP(WAZUH_URL).connect()
    try: raw_b.call("get_wazuh_agents", ATK_ARG)
    except: pass
    baseline_without = time.time() - t0b
    raw_b.disconnect()

    for i in range(n):
        # WITHOUT: injected arg reaches server — no input validation
        lat_without.append(baseline_without)
        fn += 3   # all 3 attack subtests succeed without framework

        # WITH: framework validates before any MCP call
        t0      = time.time()
        mcp, fw = make_fw(WAZUH_URL)
...
def run_t05(n):
    ...
    FLOOD     = "A"*110_000
    LEGIT_BIG = "B"*80_000   # under 100KB limit

    lat_without, lat_with = [], []
    tp=0; fp=0; tn=0; fn=0

    for i in range(n):
        # WITHOUT: evil server output reaches agent unvalidated
        t0   = time.time()
        evil = RawMCP(EVIL_URL).connect()
        try:
        ...
        # A: indirect prompt injection in output
